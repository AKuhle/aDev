    ...
    {
        {
            ...
            if (!deque->empty())
            {
                ...
                {
                    // last element founf => finish task
                    return;
                }
                else
                {
...
bool CmdBase4Tiles::execute()
{
    ...
    const shared_ptr<aLayerStack>           &pPre = pPreLS.get();
    shared_ptr<aLayerStack>                 &pPost = pPostLS.get();
    ...
    for (u32 i = 0; i < aThread::hardwareConcurrency(); i++)
    {
