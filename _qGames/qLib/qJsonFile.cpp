...
namespace qLib
{
    ...
    *******************************************************************************/
    bool qJsonFile::ReadAllValues(std::function<void(qVec<qString16>&, const QJsonValue&)> _f) const
    {
        QFile           file(m_sFilename.ToQString());
        QString         sJsonFile;
        bool            bSuccess    = false;
    ...
    /*******************************************************************************
    * qJsonFile::ReadAndParseAllValues
    *******************************************************************************/
    bool qJsonFile::ReadAndParseAllValues(std::function<void(qVec<qString16>&, qJsonValue&)> _f) const
    {
        QFile           file(m_sFilename.ToQString());
        QString         sJsonFile;
        bool            bSuccess    = false;
        ...
        if(file.open(QIODevice::ReadOnly | QIODevice::Text))
        {
            ...
            {
                // start the iteration
                ReadAndParseJsonObj(jsonDoc.object(), vecKeys, _f);
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * qJsonFile::ReadAndParseJsonObj
    *******************************************************************************/
    void qJsonFile::ReadAndParseJsonObj(const QJsonObject                                   &_obj,
                                        qVec<qString16>                                     &_vecKeys,
                                        std::function<void(qVec<qString16>&, qJsonValue&)>  _f) const
    {
        ...
        for (it = _obj.begin(); it != _obj.end(); it++)
        {
            ...
            if (it->isObject())
            {
                // if the child is another json object => next recursion
                _vecKeys.push_back(qString16::FromQString(it.key()));
                ReadAndParseJsonObj(it->toObject(), _vecKeys, _f);
                _vecKeys.pop_back();
            }
