...
//            }
    ...
    *******************************************************************************/
    void qPicBuffer::AddLayer2Buffer(qLayerI                *_pLayer,                // the layer to add
                                     ...
                                     dbl                    _dZoomFactor) const
    {
        ...
        qRect2D<s32>    rctLayer_zoom	= _pLayer->Geometry();
        rctLayer_zoom.Resize(_dZoomFactor);

        s32             s32LayerStartX  = Clamp<s32> (_v2dBufOrigin_zoom.x() - rctLayer_zoom.x(),
                                                      0, rctLayer_zoom.w() - 1);

        s32             s32LayerEndX    = Clamp<s32> (_v2dBufOrigin_zoom.x() - rctLayer_zoom.x() + m_d2dBuf.w(),
                                                      0, rctLayer_zoom.w() - 1);

        s32             s32LayerStartY  = Clamp<s32> (_v2dBufOrigin_zoom.y() - rctLayer_zoom.y(),
                                                      0, rctLayer_zoom.h() - 1);

        s32             s32LayerEndY    = Clamp<s32> (_v2dBufOrigin_zoom.y() - rctLayer_zoom.x() + m_d2dBuf.h(),
                                                      0, rctLayer_zoom.h() - 1);
...
//            } // case enumLayerType::RGB_8
        ...
        if (bValid)
        {
            ...
            for (const unique_ptr<qLayerI> &pLayer : *_pLayerStack)
            {
                if (pLayer->Visible())
                {
