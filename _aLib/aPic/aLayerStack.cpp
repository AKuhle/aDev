...
* includes
*******************************************************************************/
#include "aPic/aLayerStack.h"
#include "aPic/aLayer_i.h"
...
*******************************************************************************/
shared_ptr<aLayerStack> aLayerStack::createClone(bool   _bCopyImageData) const
{
    shared_ptr<aLayerStack> pNewLS = make_shared<aLayerStack> ();
    ...
    {
        pNewLS->addLayer(pLayer->createClone(_bCopyImageData));
    }
...
* aLayerStack::createClone
*******************************************************************************/
shared_ptr<aLayerStack> aLayerStack::createClone(s32        _s32NewW,
                                                 s32        _s32NewH) const
{
    shared_ptr<aLayerStack> pNewLS = make_shared<aLayerStack> ();

    pNewLS->setType(m_eType);
    pNewLS->setDimension(_s32NewW, _s32NewH);
    pNewLS->setName(m_sName);

    for (const unique_ptr<aLayerI> &pLayer : *this)
    {
        pNewLS->addLayer(pLayer->createClone(_s32NewW, _s32NewH));
    }
