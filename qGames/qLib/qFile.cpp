...
namespace qLib
{
    ...
    *******************************************************************************/
    ...
    {
        QFileInfo   fi(QString::fromStdU16String(_sFilename.ToStdU16String()));
    ...
    *******************************************************************************/
    void qFile::GetFilesInDirectory(const qString16&            _sDir,
                                    ...
                                    qVec<qString16>&            _vecFiles,
                                    bool                        _bFullPath  /*= true*/
                                    /*bool                      _bRecursive  = false*/)
    {
        QDir            dir(QString::fromStdU16String(_sDir.ToStdU16String()));
        QStringList     lstFilters;
        QFileInfoList   lstFileInfo;

        for (const qString16& sPattern : _lstPattern)
        {
            lstFilters << QString::fromStdU16String(sPattern.ToStdU16String());
        }
        ...
        for (QFileInfo &fi : lstFileInfo)
        {
            if (_bFullPath)
            {
                _vecFiles.push_back(qString16(fi.absoluteFilePath().toStdU16String()));
                    //fi.absoluteFilePath();
            }
            else
            {
                _vecFiles.push_back(qString16(fi.fileName().toStdU16String()));
            }
        }
    } // qFile::GetFilesInDirectory
