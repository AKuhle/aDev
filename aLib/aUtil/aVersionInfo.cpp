...
aString aVersionInfo::versionString() const
{
    ...
    sVersion = aString("V:") + aString::from_value(m_u32Major) +
               aString(".") + aString::from_value(m_u32Minor) +
               aString(".") + aString::from_value(m_u32Revision);

    if (m_u32Build > 0)
    {
        sVersion += aString("-") + aString::from_value(m_u32Build);
    }
